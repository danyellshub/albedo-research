...
sasp_lat = 37.906909
sasp_long = -107.711317
sbsp_lat = 37.906879
sbsp_long  = -107.726259
...
save(sasp_sbsp_long, file = "data/sasp_sbsp_long.Rdata")
```
...
# test <- sy2008%>%
#   filter(SY == "SY2008")%>%
#   filter(alb_unfilt < 0.3 & depth_m == 0)%>%
#   summarize(mean = mean(alb_unfilt))

ggplot(sy2008%>%filter(DOSY >61 & DOSY < 91 & hour ==12), aes(x=datetime))+
  geom_line(aes(y=alb_unfilt, color = site, linetype = site))+
  geom_point(aes(y=alb_unfilt, color = site, shape =site), size = 1)+
  geom_hline(yintercept = 0.2, linetype = "dotdash")+
  scale_color_manual(values = c("plum4", "red4"))+
  geom_line(aes(y=depth_m+0.2, linetype = site), color = "steelblue2")+
  scale_y_continuous(name = "Albedo", limits = c(0.2,1),
                     sec.axis = sec_axis(~.-0.2, name = "Depth (m)"))+
  theme_bw()+
  theme(legend.position = c(0.15,0.8), legend.title = element_blank(), text = element_text(size = 14, family = "Times New Roman"))+
  xlab(element_blank())
...
ggplot(sy2008%>%filter(hour == 12)%>%rename("Site" = site), aes(x=depth_m))+
...
ggplot(sasp_sbsp_long%>%filter(hour == 12 & !SY == "SY2005" & alb_unfilt < 0.97)%>%rename("Site" = site), aes(x=depth_m))+
  geom_point(aes(y = alb_unfilt, shape = Site, color = DOSY), size =2)+
  scale_shape_manual(values = c(1,4))+
  labs(x="Depth (m)", y = "Broadband Albedo")+
  scale_color_gradientn(colours = rainbow(6))+
...
ggplot(sy2008sasp_long%>%filter(!date == "2007-10-02"), aes(x=date))+
  ...
  scale_y_continuous(name = "Broadband Albedo",
                     sec.axis = sec_axis(~., name = "∆ Depth (m)"))+
  xlab(element_blank())+
  labs(title = expression("SASP, noon to noon:"~e^-0.02))
...
### Vrsghy Attempt Using Hourly
```{r}
hrly_sasp_vrsghy <- sasp_combined%>%
  mutate(depth_m = ifelse(is.na(depth_m),0,depth_m))%>%
  mutate(depth_diff = depth_m - lag(depth_m),
         depth_diff = ifelse(is.na(depth_diff),0,depth_diff),
         depth_diff_pos = ifelse(depth_diff >0, depth_diff, 0),
         cumsnow = cumsum(depth_diff_pos),
...
alb_loop = hrly_sasp_vrsghy$vrsghy_alb
alb_min = hrly_sasp_vrsghy$alb_min

for(i in 1:nrow(hrly_sasp_vrsghy)){
  if(is.na(alb_loop[i])){
    alb_loop[i] = ((alb_loop[i-1] - alb_min[i])*exp(-0.02)+alb_min[i])
  }
}

hrly_sasp_vrsghy$vrsghy_alb <- alb_loop

ggplot(hrly_sasp_vrsghy%>%filter(hour == 12 & SY == "SY2008" & !DOSY == 32), aes(x=date))+
  geom_line(aes(y=vrsghy_alb), linetype = "dashed", color = "red4")+
  geom_line(aes(y=alb_unfilt))+
  geom_col(aes(y=depth_diff*3), fill = "steelblue3")+
  theme_bw()+
  theme(text = element_text(size = 16, family = "Times New Roman"))+
  xlab(element_blank())+
   scale_y_continuous(name = "Broadband Albedo",
                     sec.axis = sec_axis(~./3, name = "∆ Depth (m)"))+
  labs(title = expression("SASP, hourly:"~e^-0.02))


### SBSP

hrly_sbsp_vrsghy <- sbsp_combined%>%
  filter(!date == "2008-01-01")%>%
  mutate(depth_m = ifelse(is.na(depth_m) & DOSY < 30,0,depth_m))%>%
  mutate(depth_diff = depth_m - lag(depth_m),
         depth_diff = ifelse(is.na(depth_diff),0,depth_diff),
         depth_diff_pos = ifelse(depth_diff >0, depth_diff, 0),
         cumsnow = cumsum(depth_diff_pos),
...
alb_loop = hrly_sbsp_vrsghy$vrsghy_alb
alb_min = hrly_sbsp_vrsghy$alb_min

for(i in 1:nrow(hrly_sbsp_vrsghy)){
  if(is.na(alb_loop[i])){
    alb_loop[i] = ((alb_loop[i-1] - alb_min[i])*exp(-0.02)+alb_min[i])
  }
}

hrly_sbsp_vrsghy$vrsghy_alb <- alb_loop

ggplot(hrly_sbsp_vrsghy%>%filter(hour == 12 & SY == "SY2008"), aes(x=date))+
  geom_line(aes(y=vrsghy_alb), linetype = "dashed", color = "red4")+
  geom_line(aes(y=alb_unfilt))+
  geom_col(aes(y=depth_diff*3), fill = "steelblue3")+
  theme_bw()+
  ...
  xlab(element_blank())+
   scale_y_continuous(name = "Broadband Albedo",
                     sec.axis = sec_axis(~./3, name = "∆ Depth (m)"))+
  labs(title = expression("SBSP, hourly:"~e^-0.02))
