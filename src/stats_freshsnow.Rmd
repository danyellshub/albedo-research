...
modeltest <- new_df%>%
  ...
  mutate(depth_diff = depth_m - lag(depth_m),
         ...
         albedo = ifelse(depth_m == 0, 0.3, NA))
...
for(i in 1:nrow(modeltest)){
  if(is.na(alb_model[i]) & is.na(fresh[i])){
    
     alb_vis[i] = coef(vis_lm_simp)[1]- coef(vis_lm_simp)[2]*diff[i] + coef(vis_lm_simp)[3]*log(depth[i]) + coef(vis_lm_simp)[4]*alb_model[i-1] + coef(vis_lm_simp)[5]*temp[i] + coef(vis_lm_simp)*dust[i]
     
      alb_filt[i] = coef(filt_lm_simp)[1] + coef(filt_lm_simp)[2]*diff[i] + coef(filt_lm_simp)[3]*alb_model[i-1] - coef(filt_lm_simp)[4]*temp[i] + coef(filt_lm_simp)[5]*log(depth[i])
     
     alb_model[i] = coef(lm_alb)[1] + coef(lm_alb)[2]*alb_filt[i] + coef(lm_alb)[3]*alb_vis[i]
    
  }else{if(is.na(alb_model[i] & fresh[i] == 0)){
    alb_model[i] = ((alb_model[i-1]*exp(-0.05)))
    }
  }
}
...
modeltest_xts <- modeltest%>%
  select(datetime, albedo, depth_m, alb_unfilt)%>%
