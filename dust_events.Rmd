...
events_yearly <- dust_06_14%>%
  ...
  group_by(WY)
  summarise(count = max(dust_event))
  
ggplot(events_yearly, aes(x=date))+
  geom_point(aes(y=dust_event))+
...
peak_depth <- sasp_alb_dust_filter %>%
  ...
  ungroup()%>%
  mutate(max_no_depth = ifelse(max_depth == correct_m, 700,0))
...
alb_long<- visible_sasp%>%
  ...
  gather(key = "type", value = "albedo", 
         -date, -Depth_meters, -max_no_depth,-dust_event,-SY,-DOSY)

alb_long_nir_swir <- visible_sasp%>%
  filter(DOSY<300)%>%
  select(1,5:6, 31, 28,8,22,24)%>%
  rename("Depth_meters" = correct_m,
         "NIR_SWIR" = alb_filt,
         "Visible" = alb_vis)%>%
  mutate(dust_event = dust_event*0.5,
...
alb_long_pos_all <- visible_sasp%>%
  ...
  gather(key = "type", value = "albedo", 
         ...
         -SY,-DOSY, -depth_diff, -depth_positive)
  
```

Visible vs Broadband Graphing
```{r}
ggplot(alb_long, aes(x=DOSY))+
...
filtered_dust_graphing <- filtered_dust_graphing%>%
  mutate(seq = as.numeric(as.character(seq)))%>%ss
...
ggplot(filtered_dust_graphing, aes(x=seq, group = event))+
  geom_line(aes(y=alb_vis), color = "goldenrod")+
  geom_line(aes(y=alb_final), color = "grey22")+
  ...
  theme_bw()+
  labs(x="", title = "SASP Albedo and Dust", subtitle = "Golden = Visible Albedo, Black = Broadband, Brown Lines = Dust Events" )+
...
ggplot(visible_sasp_early, aes(x=DOSY))+
  geom_line(aes(y=alb_final))+
...
ggplot(visible_sasp_late, aes(x=DOSY))+
  ...
  geom_line(aes(y=alb_final))+
  geom_col(aes(y=depth_diff_pos), fill = "black", width = 0.5)+
